...
def Slice_image(npimg, nv, rp, size, alpha = 1):
    ...
    for m in range(0, S):
        for n in range(0, S):
            try:
                if int(round(coordenadas[m, n, 0]+rp[0])) < size and int(round(coordenadas[m, n, 1]+rp[1])) < size and int(round(coordenadas[m, n, 2]+rp[2])) < size and int(round(coordenadas[m, n, 0]+rp[0])) > 0 and int(round(coordenadas[m, n, 1]+rp[1])) > 0 and int(round(coordenadas[m, n, 2]+rp[2])) > 0:
...
def Scanziani(npimg, k_sub = 40, k_line = 200, k_ROI = 40, N = 5, Parallel = 1):
    ...
    with multiprocessing.Manager() as manager:
        ...
        pool.starmap(process_segment, [(i, theta_list, R_list, npimg, (nx_R[i], ny_R[i], nz_R[i]), (IPx_R[i], IPy_R[i], IPz_R[i]), k_sub) for i in range(total_points)])
        pool.close()
        pool.join()
